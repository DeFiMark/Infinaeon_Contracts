...
interface IInfinitySwapCallee {
    function InfinitySwapCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
}
...
contract InfinitySwapPair is IInfinitySwapPair {
    ...
    string public constant override name = 'InfinitySwap LPs';
    string public constant override symbol = 'InfinitySwap-LP';
    ...
    function _transfer(address from, address to, uint value) private {
        require(
            ...
            'InfinitySwap: INSUFFICIENT_BALANCE'
        );
        require(
            to != address(0),
            'InfinitySwap: TRANSFER_TO_ZERO'
        );
        require(
            value > 0,
            'InfinitySwap: TRANSFER_ZERO'
        );
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'InfinitySwap: TRANSFER_FAILED');
    }

    // called once by the factory at time of deployment
    function initialize(address _token0, address _token1) external override {
        require(msg.sender == factory, 'InfinitySwap: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112, uint112) private {
        require(balance0 <= ~uint112(0) && balance1 <= ~uint112(0), 'InfinitySwap: OVERFLOW');
    ...
    function mint(address to) external override lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'InfinitySwap: INSUFFICIENT_LIQUIDITY_MINTED');
    ...
    function burn(address to) external override lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'InfinitySwap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override payable lock {
        require(amount0Out > 0 || amount1Out > 0, 'InfinitySwap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'InfinitySwap: INSUFFICIENT_LIQUIDITY');

        // handle gas fees associated with InfinitySwap
        {
            uint fee = IInfinitySwapFactory(factory).ethFee(address(this));
            require(msg.value >= fee, 'InfinitySwap: INSUFFICIENT FEE');
            IFeeTo(IInfinitySwapFactory(factory).feeTo()).takeFee{value: fee}(msg.sender, address(this));
        }
        ...
        require(to != _token0 && to != _token1, 'InfinitySwap: INVALID_TO');
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) IInfinitySwapCallee(to).InfinitySwapCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'InfinitySwap: INSUFFICIENT_INPUT_AMOUNT');
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
        uint balance0Adjusted = (balance0.mul(10000).sub(amount0In.mul(IInfinitySwapFactory(factory).getSwapFee())));
        uint balance1Adjusted = (balance1.mul(10000).sub(amount1In.mul(IInfinitySwapFactory(factory).getSwapFee())));
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(10000**2), 'InfinitySwap: K');
        }
...
contract InfinitySwapFactory is IInfinitySwapFactory {
    bytes32 public constant override INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(InfinitySwapPair).creationCode));
    ...
    uint256 public minEthFee = 0.002 ether;
    ...
    function createPair(address tokenA, address tokenB, uint256 _ethFee) external override returns (address pair) {
        require(tokenA != tokenB, 'InfinitySwap: IDENTICAL_ADDRESSES');
        require(_ethFee >= minEthFee, 'InfinitySwap: INSUFFICIENT FEE');
        ...
        require(token0 != address(0), 'InfinitySwap: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'InfinitySwap: PAIR_EXISTS'); // single check is sufficient

        // create pair
        bytes memory bytecode = type(InfinitySwapPair).creationCode;
        ...
        IInfinitySwapPair(pair).initialize(token0, token1);
    ...
    function changeOwner(address _owner) external {
        require(msg.sender == owner, 'InfinitySwap: FORBIDDEN');
        owner = _owner;
    }

    function setMinETHFee(uint256 newFee) external {
        require(msg.sender == owner, 'InfinitySwap: FORBIDDEN');
        require(newFee > 0, 'InfinitySwap: FEE OUT OF BOUNDS');
        minEthFee = newFee;
    }
    ...
    function setFeeTo(address newFeeTo) external {
        require(msg.sender == feeToSetter, 'InfinitySwap: FORBIDDEN');
        feeTo = newFeeTo;
    }
    ...
    function setETHFee(address pair, uint256 newFee) external {
        require(msg.sender == owner, 'InfinitySwap: FORBIDDEN');
        require(newFee >= minEthFee, 'InfinitySwap: INSUFFICIENT FEE');
        ethFee[pair] = newFee;
    }
