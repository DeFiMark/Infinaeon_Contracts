...
contract OPHDEXRouter is IOPHDEXRouter {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'OPHDEXRouter: EXPIRED');
        _;
    }
    ...
    function getETHFee(address pair) public view returns (uint256) {
        return IOPHDEXFactory(factory).ethFee(pair);
    }
    ...
    function _addLiquidity(
        ...
        if (IOPHDEXFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IOPHDEXFactory(factory).createPair(tokenA, tokenB, ethFee);
        }
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'OPHDEXRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'OPHDEXRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        liquidity = IOPHDEXPair(pair).mint(to);
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
    ...
    function removeLiquidity(
        ...
        address pair = pairFor(tokenA, tokenB);
        IOPHDEXPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IOPHDEXPair(pair).burn(to);
        (address token0,) = sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'OPHDEXRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'OPHDEXRouter: INSUFFICIENT_B_AMOUNT');
    }
    function removeLiquidityETH(
        address token,
        uint liquidity,
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IOPHDEXPair(_pair).swap{value: fee}(
                amount0Out, amount1Out, to, new bytes(0)
            );
        }
    }

    function getETHFeeForSwap(address[] memory path) public view returns (uint256 totalFee) {
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'OPHDEXRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'OPHDEXRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, pairFor(path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'OPHDEXRouter: INVALID_PATH');
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'OPHDEXRouter: INVALID_PATH');
        amounts = getAmountsIn(amountOut, path);
        require(amounts[0] <= amountInMax, 'OPHDEXRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, pairFor(path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(amounts[0] <= msg.value.sub(ethFee), 'OPHDEXRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(pairFor(path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = sortTokens(input, output);
            IOPHDEXPair pair = IOPHDEXPair(pairFor(input, output));
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'OPHDEXRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'OPHDEXRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function pairFor(address tokenA, address tokenB) public view returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                IOPHDEXFactory(factory).INIT_CODE_PAIR_HASH() // init code hash
            )))));
    }
    ...
    function getReserves(address tokenA, address tokenB) public view returns (uint reserveA, uint reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = IOPHDEXPair(pairFor(tokenA, tokenB)).getReserves();
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) public view returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.mul(IOPHDEXFactory(factory).getDenomFee());
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public view returns (uint amountIn) {
        ...
        uint denominator = reserveOut.sub(amountOut).mul(IOPHDEXFactory(factory).getDenomFee());
