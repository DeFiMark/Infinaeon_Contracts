...
contract NativeSwapperPolygon is Ownable, AxelarExecutable {
    ...
    /** Events */
    event SwapSuccessful(address indexed user, uint256 amount);
    event SwapFailed(address indexed user, uint256 amount, uint256 minOut, uint256 amountAUSDC);
    ...
    function _executeWithToken(
        ...
        (address to, uint112 percentForGas, uint256 minOut) = abi.decode(payload, (address, uint112, uint256));

        // fetch usdc balance
        uint256 balance = IERC20(aUSDC).balanceOf(address(this));

        // send aUSDC to swapper
        IERC20(aUSDC).approve(swapper, balance);

        // perform low-level call on swapper
        (bool success,) = swapper.call(abi.encodeWithSignature("buyFor(address,uint256,uint112,uint256)", to, balance, percentForGas, minOut));
        if (!success) {
            uint256 newBal = IERC20(aUSDC).balanceOf(address(this));
            // send them usdc instead
            if (newBal >= balance) {
                // send aUSDC to user
                IERC20(aUSDC).transfer(to, balance);
            } else {
                // send all available aUSDC to user
                IERC20(aUSDC).transfer(to, newBal);
            }
            emit SwapFailed(to, balance, minOut, newBal);
        } else {
            emit SwapSuccessful(to, balance);
        }
    }
